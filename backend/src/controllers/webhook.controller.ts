...
import { createGoal, GoalCategory } from '../models/goal.model';
import { findUserById } from '../models/user.model';
import { triggerWhatsAppSummary } from '../services/pipedream.service';
...
const extractGoalsFromTranscript = (transcript: string, summary: string): Array<{
  title: string;
  description: string;
...
export const vapiConversationEndHandler = async (req: Request, res: Response) => {
  try {
    ...
    const extractedGoals = extractGoalsFromTranscript(
      payload.transcript,
      payload.summary
    );
